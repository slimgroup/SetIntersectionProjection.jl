...
for i=1:size(d_obs,1)
    ...
    [x_tv,out_tv] = solver_sBPDN_W( FWD_OP, W_tv, b, EPS, mu, x0(:), z0, opts_copy, contOpts);
    ...
    pause(1)
    TFOCS_TV_inpainting(i,:,:)=reshape(x_tv,n1,n2);
   
    SNR=@(in1,in2) 20*log10(norm(in1)/norm(in1-in2))
    SNR(vec(m_evaluation(i,:,:)),x_tv)
end

 save('TFOCS_TV_inpainting','TFOCS_TV_inpainting')
