...
image = convert(Array{Float32,2},image)
image = image[1:4:end,1:4:end]
image = repeat(image,outer=[1,1,9,1]) #make image 9 channels for the invertible network
...
for j=1:2 #loop over active output channels (classes)
  if j==1
    ...
    #cardinality:
    m_min     = 0
    m_max     = round(Int,0.5*prod(n[1:2]))
  ...
  elseif j==2
    ...
    #cardinality:
    m_min     = 0
    m_max     = round(Int,0.90*prod(n[1:2]))
    set_type  = "cardinality"
    TD_OP     = "identity"
    app_mode  = ("matrix","")
    custom_TD_OP = ([],false)
...
savefig("prediction_cameraman_Channels.png")
...
savefig("prediction_cameraman.png")

#Plot data+prediction
figure(figsize=(4,4))
imshow(Array(pred_thres)[2:end-1,2:end-1])
imshow(-image[2:end-1,2:end-1,3],alpha=0.6,cmap="Greys")
ax = gca()
