...
if parallel == true
  ...
  #mul!(rhs,TD_OP',rho .* y .+ l) #not possible
  #rhs = TD_OP'*(rho .* y .+ l) possible but slow
else #serial
  fill!(rhs,TF(0));
  if Blas_active #serial with explicit BLAS calls
    temp_array = zeros(TF,length(rhs))
    for ii=1:p
      mul!(temp_array,TD_OP[ii]',rho[ii] .* y[ii] .+ l[ii])
      #BLAS.axpy!(TF(1.0), TD_OP[ii]'*(rho[ii] .* y[ii] .+ l[ii]), rhs)
      BLAS.axpy!(TF(1.0), temp_array, rhs)
    end
  else
